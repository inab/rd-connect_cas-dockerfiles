...
if [ $# -ge 2 ] ; then
	...
	if [ -r "${backupArchive}" -a -r "${mappingsFile}" ] ; then
		...
		docker_migrate_volumes "${prefix}" "${backupArchive}" "${mappingsFile}" "${tvol}"
		
		echo "To create the instances, run ./createInstances.sh ${origPrefix}"
	else
		echo "ERROR: backup archive $backupArchive or mappings file ${mappingsFile} do not exist!" 1>&2
		exit 1
	fi
else
	echo "Usage: $0 {migrated volumes prefix} {backup archive} {mappings file}"
fi
